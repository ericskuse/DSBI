...
ct_model<-rpart(play~gender+salary+education+marital_status,           # model formula
                data=adult,                             # dataset
...
```
```{r}
adult$ct_pred_prob<-ct_pred[,2]   

adult$ct_pred_class<-ifelse(adult$ct_pred_prob>0.4,"Yes","No")   
```

```{r}
adult%>%
  filter(ct_pred_prob>0.3) 
